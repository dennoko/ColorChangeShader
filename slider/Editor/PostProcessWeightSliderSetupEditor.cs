...
/// <summary>
/// PostProcessWeightSlider のセットアップを自動化するエディタ拡張。
/// Knob オブジェクト (PostProcessWeightSlider が付いているオブジェクト) を選択した状態で
/// Inspector のボタンを押すとセットアップが完了する。
/// </summary>
[CustomEditor(typeof(PostProcessWeightSlider))]
public class PostProcessWeightSliderSetupEditor : Editor
{
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();

        PostProcessWeightSlider slider = (PostProcessWeightSlider)target;
        ...
        EditorGUILayout.HelpBox(
            "階層の確認:\n" +
            "SliderParent (空)\n" +
            "  ├─ Knob ← このオブジェクト (PostProcessWeightSlider + VRC_Pickup etc.)\n" +
            "  ├─ track_start\n" +
            "  └─ track_end\n\n" +
    ...
    void AutoCreateTrack(PostProcessWeightSlider slider)
    {
        ...
        if (parent == null)
        {
            EditorUtility.DisplayDialog(
                "PostProcessWeightSlider セットアップ",
        ...
        if (startProp.objectReferenceValue == null)
        {
            ...
            Debug.Log($"[PostProcessWeightSlider] track_start を '{parent.name}' 下に作成");
        }
        ...
        if (endProp.objectReferenceValue == null)
        {
            ...
            Debug.Log($"[PostProcessWeightSlider] track_end を '{parent.name}' 下に作成");
        }
    ...
    void AutoAddComponents(PostProcessWeightSlider slider)
    {
        Undo.RecordObject(slider.gameObject, "PostProcessWeightSlider Auto Add Components");
